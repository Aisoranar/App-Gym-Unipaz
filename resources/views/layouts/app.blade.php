...
<head>
  ...
  <!-- FullCalendar estilos (solo si la vista hija los define) -->
  @yield('styles')
...
<body>
  ...
  <script>
    ...
    function updateDesktopToggle() {
      ...
      if (window.innerWidth >= 768) desktopToggle.style.display = 'inline-block';
      else desktopToggle.style.display = 'none';
    }
    ...
    // Cerrar sidebar móvil al click fuera
    document.addEventListener('click', function(e) {
      const sidebar = document.getElementById('sidebar');
      const toggle = document.getElementById('mobileToggle');
      if (sidebar.classList.contains('mobile-active') && !sidebar.contains(e.target) && e.target !== toggle) {
        sidebar.classList.remove('mobile-active');
      }
    });

    // Cerrar sidebar móvil al click en enlace
